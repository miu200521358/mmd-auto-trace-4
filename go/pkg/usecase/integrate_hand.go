...
func IntegrateHand(allFrames []*model.Frames, all4dRotateMotions []*vmd.VmdMotion, allMpRotateMotions []*vmd.VmdMotion) []*vmd.VmdMotion {
	...
	for i := range all4dRotateMotions {
		...
		go func(i int, frames *model.Frames, dRotMotion *vmd.VmdMotion, mpRotMotion *vmd.VmdMotion) {
			defer wg.Done()
			rotMotion := integrateHandMotion(frames, dRotMotion, mpRotMotion, bar)
...
func integrateHandMotion(frames *model.Frames, dRotateMotion *vmd.VmdMotion, mpRotateMotion *vmd.VmdMotion, bar *pb.ProgressBar) *vmd.VmdMotion {
